...
// ProvidersHandler is the tools.audio_providers process handler.
func ProvidersHandler(proc *process.Process) interface{} {
	capability := proc.ArgsString(0, "audio")
...
// findFirstSTTConnector returns the connector ID of the first available
// speech-to-text provider, or empty string if none found.
func findFirstSTTConnector(authInfo *oauthTypes.AuthorizedInfo) string {
	if llmprovider.Global == nil {
		return ""
	}
	providers, err := listProvidersByCapability("audio", authInfo)
