...
func TestExecutionStoreGet(t *testing.T) {
	...
	setupTestExecution(t, s, ctx, identity)
...
func TestExecutionStoreList(t *testing.T) {
	...
	setupTestExecutionsForList(t, s, ctx, identity)
...
func TestExecutionStoreListResults(t *testing.T) {
	...
	setupTestResultsData(t, s, ctx, identity)
...
func TestExecutionStoreListActivities(t *testing.T) {
	...
	setupTestActivitiesData(t, s, ctx, identity)
	...
	t.Run("filters_by_since", func(t *testing.T) {
		...
		require.NoError(t, err)
		assert.GreaterOrEqual(t, len(activitiesAll), 3)
		...
		require.NoError(t, err)
		assert.Equal(t, 0, len(activitiesFuture))
	})
	...
	t.Run("filters_by_type_completed", func(t *testing.T) {
		...
		for _, a := range activities {
			assert.Equal(t, store.ActivityExecutionCompleted, a.Type)
		}
	...
	t.Run("filters_by_type_failed", func(t *testing.T) {
		...
		for _, a := range activities {
			assert.Equal(t, store.ActivityExecutionFailed, a.Type)
		}
		assert.Greater(t, len(activities), 0)
	})
...
func setupTestExecution(t *testing.T, s *store.ExecutionStore, ctx context.Context, identity *testprepare.TestIdentity) {
	t.Helper()
...
func setupTestExecutionsForList(t *testing.T, s *store.ExecutionStore, ctx context.Context, identity *testprepare.TestIdentity) {
	t.Helper()
	startTime := time.Now()
...
func setupTestActivitiesData(t *testing.T, s *store.ExecutionStore, ctx context.Context, identity *testprepare.TestIdentity) {
	t.Helper()
	startTime := time.Now().Add(-2 * time.Hour)
	endTime := time.Now().Add(-1 * time.Hour)
