...
func NewTestAdapterWithBot(memberID, appID, appSecret string) *TestAdapter {
	ta := NewTestAdapter()
	ta.a.bots[memberID] = &botEntry{
		robotID:   memberID,
		appID:     appID,
		appSecret: appSecret,
		bot:       fsapi.NewBot(appID, appSecret),
	}
	return ta
}
...
func (ta *TestAdapter) GetBot(robotID string) *fsapi.Bot {
	ta.a.mu.RLock()
...
func (ta *TestAdapter) HandleMessagesForTest(ctx context.Context, robotID string, cms []*fsapi.ConvertedMessage) {
	ta.a.mu.RLock()
