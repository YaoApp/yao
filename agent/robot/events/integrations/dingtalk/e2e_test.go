...
func TestDingTalkAdapter(t *testing.T) {
	...
	t.Run("Apply", func(t *testing.T) {
		...
		bot := a.GetBot("robot_e2e_dt_adapter")
	...
	t.Run("Apply_Update", func(t *testing.T) {
		...
		a.Apply(context.Background(), robot)
		require.NotNil(t, a.GetBot("robot_e2e_dt_update"))

		a.Apply(context.Background(), robot)
		assert.Equal(t, 1, a.BotCount())

		a.Remove(context.Background(), "robot_e2e_dt_update")
		assert.Nil(t, a.GetBot("robot_e2e_dt_update"), "bot should be removed")
	...
	t.Run("HandleMessages", func(t *testing.T) {
		a := dingtalk.NewTestAdapterWithBot("robot_e2e_dt_handle", clientID, clientSecret)
		defer a.Close()
		...
		a.HandleMessagesForTest(ctx, "robot_e2e_dt_handle", cms)

		assert.False(t, a.MarkSeen("dt:robot_e2e_dt_handle:test_msg_1"),
	...
	t.Run("ApplyDisabled", func(t *testing.T) {
		...
		a.Apply(context.Background(), robot)
		assert.Nil(t, a.GetBot("robot_e2e_dt_disabled"), "disabled bot should not be registered")
