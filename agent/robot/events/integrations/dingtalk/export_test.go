...
func NewTestAdapterWithBot(memberID, clientID, clientSecret string) *TestAdapter {
	ta := NewTestAdapter()
	ta.a.bots[memberID] = &botEntry{
		robotID:      memberID,
		clientID:     clientID,
		clientSecret: clientSecret,
		bot:          dtapi.NewBot(clientID, clientSecret),
	}
	return ta
}
...
func (ta *TestAdapter) GetBot(robotID string) *dtapi.Bot {
	ta.a.mu.RLock()
...
func (ta *TestAdapter) HandleMessagesForTest(ctx context.Context, robotID string, cms []*dtapi.ConvertedMessage) {
	ta.a.mu.RLock()
