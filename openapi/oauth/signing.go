...
// knownDefaultKeyFingerprint is the SHA-256 fingerprint of the default signing
// public key shipped in yao-init/openapi/certs/signing-key.pem. All default
// deployments share this key. When detected at startup the engine auto-generates
// a unique replacement and persists it back to the same path.
const knownDefaultKeyFingerprint = "4ffd27458d330eb745582df5d1216e8df7e014013ba2247f229a8028bdfd2665"
...
func loadExistingCertificates(certs *SigningCertificates, config *types.SigningConfig) error {
	...
	// Detect and replace the well-known default signing key shipped with yao-init.
	if signer, ok := certs.SigningKey.(crypto.Signer); ok {
		pubDER, err := x509.MarshalPKIXPublicKey(signer.Public())
		if err == nil {
			fingerprint := sha256.Sum256(pubDER)
			if hex.EncodeToString(fingerprint[:]) == knownDefaultKeyFingerprint {
				fmt.Println("WARNING: Default signing key detected. Generating unique key pair...")
				newCerts, err := generateAndReplaceCertificates(config)
				if err != nil {
					fmt.Printf("WARNING: Failed to replace default key (%v). Continuing with default key. Replace manually for production.\n", err)
					return nil
				}
				certs.SigningCert = newCerts.SigningCert
				certs.SigningKey = newCerts.SigningKey
				certs.SigningKeyPair = newCerts.SigningKeyPair
				certs.SigningCertPath = newCerts.SigningCertPath
				certs.SigningKeyPath = newCerts.SigningKeyPath
				fmt.Println("Default signing key replaced. New key pair saved to:", config.SigningKeyPath)
				return nil
			}
		}
	}
...
// generateAndReplaceCertificates generates a new key pair and overwrites the
// files at the configured paths so the replacement persists across restarts.
func generateAndReplaceCertificates(config *types.SigningConfig) (*SigningCertificates, error) {
	...
	if err := os.WriteFile(config.SigningCertPath, certPEM, 0644); err != nil {
		return nil, fmt.Errorf("failed to write certificate: %w", err)
	}
	if err := os.WriteFile(config.SigningKeyPath, keyPEM, 0600); err != nil {
	...
	return &SigningCertificates{
		...
		Algorithm:       config.SigningAlgorithm,
		SigningCertPath: config.SigningCertPath,
